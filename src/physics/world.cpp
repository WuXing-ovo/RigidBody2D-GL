...
void World::set_gravity(double gx, double gy) { gravity = {gx, gy}; }

void World::set_gravity(const vec2 &g) { gravity = g; }
...
void World::add_random_Ball(const vec2 &v_min, const vec2 &v_max,
                            double mass_min, double mass_max, double size_min,
                            double size_max) {
  // Create a ball
  Ball ball_input;
  ...
  std::uniform_real_distribution<double> position_x(
      ball_input.get_size(), world_size.x - ball_input.get_size());
  std::uniform_real_distribution<double> position_y(
      ball_input.get_size(), world_size.y - ball_input.get_size());
...
void World::add_random_Balls(size_t num, const vec2 &v_min, const vec2 &v_max,
                             double mass_min, double mass_max, double size_min,
                             double size_max) {
...
// Getters
const vec2 &World::get_size() const { return world_size; }

const vec2 &World::get_gravity() const { return gravity; }
      ...
      for (auto &each_ball_j : balls) {
        ...
        total_gravity +=
            ...
            ((each_ball_i.get_position() - each_ball_j.get_position())
             ...
             0.1) *
            (each_ball_j.get_position() - each_ball_i.get_position())
                .normalized();
      }
      each_ball_i.set_velocity(each_ball_i.get_velocity() +
                               total_gravity * (1 / each_ball_i.get_mass()) *
                                   dt);
    }
  }
  for (auto &each_ball : balls) {
    each_ball.set_velocity(each_ball.get_velocity() + gravity * dt);
    each_ball.set_position(each_ball.get_position() +
                           each_ball.get_velocity() * dt);
  }
}
...
void World::collision_detect() {
  ...
  std::sort(balls.begin(), balls.end(), [](const Ball &a, const Ball &b) {
    return (a.get_position().x - a.get_size()) <
           (b.get_position().x - b.get_size());
  });
#pragma omp parallel for shared(balls)
  for (size_t i = 0; i < balls.size(); i++) {
    ...
    if (each_ball.get_position().x - each_ball.get_size() < 0) {
      each_ball.set_velocity(-each_ball.get_velocity().x,
                             each_ball.get_velocity().y);
    }
...
void World::update_velocity(Ball &ball_1, Ball &ball_2) {
  ...
  const vec2 normal_vector =
      (ball_1.get_position() - ball_2.get_position()).normalized();
  ...
  // vice versa
  const auto v1_f_normal =
      (ball_1.get_mass() * v1_i_normal + ball_2.get_mass() * v2_i_normal +
       restitution * ball_2.get_mass() * (v2_i_normal - v1_i_normal)) /
      (ball_1.get_mass() + ball_2.get_mass());
  const auto v2_f_normal =
      (ball_1.get_mass() * v1_i_normal + ball_2.get_mass() * v2_i_normal +
...
void World::print_state(size_t start, size_t end) {
  ...
  std::cout << "=== World State (Frame: " << frame << ") ===" << std::endl;
  for (auto ball :
       std::span<const Ball>(balls.begin() + start, balls.begin() + end)) {
    std::cout << "Ball " << ball.get_ID() << ": pos=" << ball.get_position()
              << " vel=" << ball.get_velocity() << " mass=" << ball.get_mass()
              << " r=" << ball.get_size() << std::endl;
  }
...
void World::set_n_body_gravity(bool status) { enable_n_body_gravity = status; }
